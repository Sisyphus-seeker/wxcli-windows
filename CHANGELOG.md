...
- Add Windows x64 process discovery, installation checks, data-root discovery,
  and read-only WeChat process-memory access
- Validate database-key candidates against real database salts and support
  per-database encrypted key material
- Add Windows account-directory resolution, platform-native app paths, polling
  fallback, local server management, and stale-cache query fallback
- Verify compatibility with WeChat 4.1.11.24

### Release

- Publish an unofficial Windows-compatible distribution derived from
  `pandorafuture/wx-cli`
- Remove the experimental image OCR feature because Windows WeChat V2 images
  cannot be decoded without a separately supplied image key
- Package the license and README with the Windows binary and publish SHA256
  checksums

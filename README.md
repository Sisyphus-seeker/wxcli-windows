...
| 微信版本 | 消息读取与查询 | `key extract` | 说明 |
|----------|----------------|---------------|------|
| 4.1.11.x、4.1.12.26、4.1.13.12、4.1.15.10 | ✅ | ✅ | 已验证 4.1.11.24、4.1.12.26、4.1.13.12、4.1.15.10 |
| 4.1.13.65 | ✅ | ❌ | 使用此前已保存且仍有效的密钥，可正常查询消息、解析联系人并刷新解密缓存 |
...
前往 [Releases](https://github.com/Sisyphus-seeker/wxcli-windows/releases/latest)
下载 `wx-cli-<版本>-windows-x64.zip`，解压后运行：
...
Windows 编译产物位于 `target/release/wx-cli.exe`。
...
`key extract` 扫描 Windows 微信进程内存中的数据库密钥，并按数据库盐值保存。微信
4.1.12.26、4.1.13.12 或 4.1.15.10 已打开数据库后可能无法被动取得密钥；此时请彻底退出微信，
再从管理员 PowerShell 执行 `key extract`。wx-cli 会启动微信并捕获首次数据库初始化，
登录完成后自动保存通过数据库验证的密钥。

微信 4.1.13.65 目前尚不支持 `key extract`。如果本机已有此前保存且仍有效的密钥，
`query`、`sessions`、`contacts` 等读取功能已经验证可用；首次使用、密钥缺失或密钥失效
时，暂时无法在该微信版本上完成密钥提取。

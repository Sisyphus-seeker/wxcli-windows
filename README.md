...
- **Windows**：x64，微信 4.1.11.x 和 4.1.12.26（已验证 4.1.11.24、4.1.12.26）
...
4.1.12.26 已打开数据库后可能无法被动取得密钥；此时请彻底退出微信，再从管理员
PowerShell 执行 `key extract`。wx-cli 会启动微信并捕获首次数据库初始化，登录完成后
自动保存通过数据库验证的密钥。

...
pub fn open_db_core(
    ...
    if acct.raw_key.is_some() && wx_context::direct_encrypted_open_available() {
...
/// Open session data, falling back to an empty contact index when only session.db is available.
pub fn open_db_sessions(
    acct: &AccountContext,
    progress: impl Fn(wx_context::DecryptProgress) + Send + Sync,
) -> Result<(wx_db::WechatDb, Option<DecryptStats>), Box<dyn std::error::Error>> {
    if acct.raw_key.is_some() && wx_context::direct_encrypted_open_available() {
        return open_db_core(acct, progress);
    }

    let cache = PersistentCache::new(acct, wx_decrypt::platform_default_params())?;
    let stats = DecryptRequest::new()
        .core()
        .execute_with_progress(&cache, progress)?;
    let contact_path = cache.decrypted_root().join("contact/contact.db");
    let db = if contact_path.exists() {
        wx_db::WechatDb::open_core(cache.decrypted_root())?
    } else {
        eprintln!("warning: contact database unavailable; displaying raw session identifiers");
        wx_db::WechatDb::open_sessions(cache.decrypted_root())?
    };
    Ok((db, Some(stats)))
}
...
pub fn open_db_all(
    ...
    if acct.raw_key.is_some() && wx_context::direct_encrypted_open_available() {
        ...
        let contact_path = cache.decrypted_root().join("contact/contact.db");
        let db = if contact_path.exists() {
            wx_db::WechatDb::open(cache.decrypted_root())?
        } else {
            eprintln!("warning: contact database unavailable; displaying raw contact identifiers");
            wx_db::WechatDb::open_without_contacts(cache.decrypted_root())?
        };
...
pub fn decrypt_progress_callback(event: wx_context::DecryptProgress) {
    match event {
        ...
        wx_context::DecryptProgress::Failed { path, error }
            if std::env::var_os("WX_CLI_DECRYPT_DIAGNOSTICS").is_some() =>
        {
            eprintln!("  ERR {path}: {error}");
        }
        _ => {}
    }
}

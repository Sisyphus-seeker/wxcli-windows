...
fn load_local_query(
    ...
    let (db, _cache, _stats) = if acct.raw_key.is_some()
        && wx_context::direct_encrypted_open_available()
    {
        ...
        let core_stats = match DecryptRequest::new()
            .core()
            .execute_with_progress(&cache, crate::util::decrypt_progress_callback)
        {
            Ok(stats) => Some(stats),
            Err(err) if cached_session_exists(cache.decrypted_root()) => {
                eprintln!("warning: database refresh failed; querying stale cache ({err})");
                None
            }
            Err(err) => return Err(err.into()),
        };
        ...
        let stats = match refresh {
            Ok(stats) => {
                print_cache_stats(&stats);
                Some(stats)
            }
            Err(err) if cached_message_exists(cache.decrypted_root()) => {
                eprintln!("warning: message refresh failed; querying stale cache ({err})");
                core_stats
            }
            Err(err) => return Err(err.into()),
        };
...
fn cached_session_exists(root: &std::path::Path) -> bool {
    root.join("session").join("session.db").is_file()
}

fn cached_message_exists(root: &std::path::Path) -> bool {
    root.join("message")
        .read_dir()
        .ok()
        .into_iter()

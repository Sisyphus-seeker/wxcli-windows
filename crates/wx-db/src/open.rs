...
pub(crate) fn open_connection(
    ...
    if sqlcipher_key.is_some() {
        return Err(DbError::EncryptionKey(
            "direct encrypted open is unavailable on this platform; decrypt to cache first".into(),
        ));
    }
...
impl WechatDb {
    ...
    /// Open session.db with an empty in-memory contact database.
    ///
    /// This supports session-only key captures while preserving the same query API.
    pub fn open_sessions(path: impl AsRef<Path>) -> Result<Self, DbError> {
        Self::open_with_empty_contacts(path.as_ref(), false)
    }

    /// Open available session and message databases with an empty contact index.
    pub fn open_without_contacts(path: impl AsRef<Path>) -> Result<Self, DbError> {
        Self::open_with_empty_contacts(path.as_ref(), true)
    }

    fn open_with_empty_contacts(path: &Path, scan_message_shards: bool) -> Result<Self, DbError> {
        if !path.exists() {
        ...
        let session_conn = open_connection(&session_path, None)?;

        let contact_conn = Connection::open_in_memory()?;
        contact_conn.execute_batch(
        ...
        Ok(Self {
            contact_conn,
            contact_path: PathBuf::new(),
            session_conn,
            session_path,
            shards: discover_message_shards(path, None, scan_message_shards)?,
            message_fts_path: existing_file(path.join("message/message_fts.db")),
            contact_fts_path: None,
            pool: None,
            sqlcipher_key: None,
    ...
    fn open_internal(
        ...
        let shards = discover_message_shards(path, key_ref, scan_message_shards)?;
        ...
        Ok(WechatDb {
            ...
            session_path,
            shards,
            message_fts_path: existing_file(msg_dir.join("message_fts.db")),
            contact_fts_path: existing_file(msg_dir.join("contact_fts.db")),
            pool: None,
            sqlcipher_key,
            label_cache: RwLock::new(None),
        })
...
fn discover_message_shards(
    path: &Path,
    sqlcipher_key: Option<&SqlcipherKey>,
    enabled: bool,
) -> Result<Vec<MessageShard>, DbError> {
    let msg_dir = path.join("message");
    if !enabled || !msg_dir.is_dir() {
        return Ok(Vec::new());
    }
    ...
    let mut entries: Vec<PathBuf> = std::fs::read_dir(&msg_dir)?
        ...
        .filter(|path| is_numbered_message_shard(path))
        .collect();
    entries.sort();

    let mut shards = entries
        .into_iter()
        .map(|path| MessageShard {
            start_unix: read_shard_timestamp(&path, sqlcipher_key),
            path,
            end_unix: 0,
        })
        .collect::<Vec<_>>();
    shards.sort_by_key(|shard| shard.start_unix);

    for index in 0..shards.len() {
        shards[index].end_unix = shards
            .get(index + 1)
            .map_or(i64::MAX, |next| next.start_unix - 1);
    }
    Ok(shards)
}
...
mod tests {
    ...
    #[test]
    fn open_sessions_uses_empty_contact_database() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("db_storage");
        std::fs::create_dir_all(root.join("session")).unwrap();
        Connection::open(root.join("session/session.db")).unwrap();

        let db = WechatDb::open_sessions(&root).unwrap();
        let count: i64 = db
            .contact_conn
            .query_row("SELECT COUNT(*) FROM contact", [], |row| row.get(0))
            .unwrap();
        assert_eq!(count, 0);
        assert_eq!(db.session_path, root.join("session/session.db"));
    }

    #[test]
    fn open_without_contacts_discovers_message_shards() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("db_storage");
        std::fs::create_dir_all(root.join("session")).unwrap();
        std::fs::create_dir_all(root.join("message")).unwrap();
        Connection::open(root.join("session/session.db")).unwrap();
        let shard = Connection::open(root.join("message/message_0.db")).unwrap();
        shard
            .execute_batch(
                "CREATE TABLE Timestamp (timestamp INTEGER); INSERT INTO Timestamp VALUES (1234);",
            )
            .unwrap();

        let db = WechatDb::open_without_contacts(&root).unwrap();
        assert_eq!(db.shards.len(), 1);
        assert_eq!(db.shards[0].start_unix, 1234);
    }
    ...
    #[cfg(not(target_os = "macos"))]
    #[test]
    fn open_connection_with_key_reports_unsupported_platform() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("test.db");
        Connection::open(&path).unwrap();

        let error = open_connection(&path, Some(&SqlcipherKey::new([0xAB; 32]))).unwrap_err();
